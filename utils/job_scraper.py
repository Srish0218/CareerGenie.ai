...
class LinkedInScraper(JobScraper):
    ...
    def fetch_jobs(self):
        ...
        for job_title in self.search_terms:
            linkedin_url = f"{self.BASE_URL_jobs}{job_title.replace(' ', '%20')}"
            ...
            if response.status_code == 200:
                ...
                for job in jobs:
                    ...
                    title = "Security issue by company hide title" if "********" in title else title
    ...
    def fetch_people(self):
        """Scrape LinkedIn People postings."""
        all_jobs = []
        for job_title in self.search_terms:
            linkedin_url = f"{self.BASE_URL_people}{job_title.replace(' ', '%20')}&origin=SWITCH_SEARCH_VERTICAL&sid=5GE"
            response = requests.get(linkedin_url, headers=self.HEADERS)
